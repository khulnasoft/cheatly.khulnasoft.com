...
GITHUB_REPOSITORY = {
    ...
    "cheatly.khulnasoft.comeets"      :   'khulnasoft/cheatly.khulnasoft.comeets',
    "cheatly.khulnasoft.comeets dir"  :   'khulnasoft/cheatly.khulnasoft.comeets',
...
def visualize(answer_data, request_options):
    ...
    editable = (len(answers) == 1 and answers[0]['topic_type'] == 'cheatly.khulnasoft.comeets')
...
def _render_html(query, result, editable, repository_button, topics_list, request_options):
    ...
    title = "<title>cheatly.khulnasoft.com/%s</title>" % query
    ...
    curl_line = "<span class='pre'>$ curl cheatly.khulnasoft.com/</span>"
    if query == ':firstpage':
        query = ""
    ...
    if editable:
        ...
        if '/' in query:
            query = '_' + query
        edit_page_link = 'https://github.com/khulnasoft/cheatly.khulnasoft.comeets/edit/master/sheets/' + query
        edit_button = (
